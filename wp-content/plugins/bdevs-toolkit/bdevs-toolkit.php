...
define( 'BDEVS_TOOLKIT_VER', '1.0.3' );
...
function bdevs_custom_post_types() {
    ...
    $eduman_ev_slug = get_theme_mod('eduman_ev_slug','ourevent');
    $eduman_port_slug = get_theme_mod('eduman_port_slug','ourportfolio');



    return [
        'bdevs-event' => [ 'title' => 'Event', 'plural_title' => 'Event', 'rewrite' => $eduman_ev_slug, 'menu_icon' => 'dashicons-admin-generic' ],
        'bdevs-portfolio'    => [ 'title' => 'Portfolio', 'plural_title' => 'Portfolio', 'rewrite' => $eduman_port_slug, 'menu_icon' => 'dashicons-format-image' ],
    ];
}
...
function shop_search_callback() {
    ...
    <div class="sidebar__widget-content"><div class="sidebar__search-wrapper">
        <form class="sidebar-widget-form" action="<?php print esc_url(home_url('/courses')); ?>" method="get">
            <input type="text" value="<?php print esc_attr( get_search_query() ) ?>" required name="s" placeholder="<?php echo esc_html__('Search','eduman'); ?>">
